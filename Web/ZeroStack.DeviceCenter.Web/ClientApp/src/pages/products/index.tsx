...
export default () => {
    ...
    const batchDelete = async (selectRows: API.ProductGetResponseModel[]) => {
        if (selectRows) {
            ...
            try {
                await Promise.all(selectRows.map(async p => {
                    if (p.id) {
        ...
        <PageContainer>
            <ProTable<API.ProductGetResponseModel>
                ...
                toolbar={{
                    actions: [
                        <Tooltip title="导出">
                            <Button type="link" icon={<DownloadOutlined style={{ fontSize: 20 }} onClick={() => alert('ok')} />} />
                        </Tooltip>
                    ],
                }}
            ...
            {
                selectedRows?.length > 0 && <FooterToolbar extra={
                    <div>
